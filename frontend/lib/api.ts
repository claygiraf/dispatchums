...
export const api = {
  ...
  async updateProfile(token: string, profileData: {
    ...
    const response = await fetch(`${API_V1}/auth/update-profile`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
  ...
  async forgotPassword(personalEmail: string): Promise<any> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.detail || 'Failed to send temporary password');
    }
