...
def register_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Check if username already exists (using direct query since get_user_by_username not in auth module)
...
def login_user(user_credentials: UserLogin, db: Session = Depends(get_db)):
    ...
    # Create access token with dispatcher_id (which matches what get_current_user expects)
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = create_access_token(
        data={"sub": user.dispatcher_id}, expires_delta=access_token_expires
    )
