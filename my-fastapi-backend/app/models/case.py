...
class Case(Base):
    ...
    dispatcher_location = Column(String(255), nullable=True)  # MECC or other location
    dispatcher_city = Column(String(100), nullable=True)
    dispatcher_state = Column(String(100), nullable=True)
    dispatcher_postcode = Column(String(20), nullable=True)
    
    # Resource/Ambulance assignment
    resource_id = Column(String(50), nullable=True)  # Auto-assigned resource starting 00001
    ambulance = Column(String(100), nullable=True)  # Ambulance unit assigned
    
    # Case metrics
    case_duration = Column(Integer, nullable=True)  # Duration in seconds (auto-detect)
    time_to_dispatch = Column(Integer, nullable=True)  # Time from case start to dispatch in seconds
    dispatch_time = Column(DateTime(timezone=True), nullable=True)  # When dispatch determination was made
    feedback = Column(Text, nullable=True)  # Feedback from caller or responder
    
    # Case summary details
    case_summary = Column(Text, nullable=True)  # JSON string of complete case summary
    
    # Timeline timestamps for each phase
    case_entry_time = Column(DateTime(timezone=True), nullable=True)  # When case entry was completed
    key_questions_time = Column(DateTime(timezone=True), nullable=True)  # When KQ were completed
    pdi_time = Column(DateTime(timezone=True), nullable=True)  # When PDI was completed
    dls_time = Column(DateTime(timezone=True), nullable=True)  # When DLS was completed
...
class CaseCreate(CaseBase):
    ...
    dispatcher_location: Optional[str] = None
    dispatcher_city: Optional[str] = None
    dispatcher_state: Optional[str] = None
    dispatcher_postcode: Optional[str] = None
    ...
    case_entry_time: Optional[datetime] = None
    key_questions_time: Optional[datetime] = None
    pdi_time: Optional[datetime] = None
    dls_time: Optional[datetime] = None
